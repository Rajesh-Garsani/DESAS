...
def guard_dashboard(request):
    assignments = DutyAssignment.objects.filter(guard=request.user).select_related('event')

    # Check if guard is approved
    is_approved = False
    try:
...
def assignments(request):
    ...
    return render(request, 'guards/assignments.html', context)
