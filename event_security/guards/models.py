...
class SecurityGuardProfile(models.Model):
    ...
    def save(self, *args, **kwargs):
        # Set user role to security_guard when profile is created
...
class DutyAssignment(models.Model):
    ...
    guard = models.ForeignKey(User, on_delete=models.CASCADE, related_name='assignments')
    assignment_date = models.DateTimeField(auto_now_add=True)
    details = models.TextField(blank=True)

    def __str__(self):
        return f"{self.guard.username} assigned to {self.event.name}"