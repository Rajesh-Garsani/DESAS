...
class PerPathSessionMiddleware(SessionMiddleware):
    """
    Session middleware that uses different cookie names depending on request.path.
    Admin path (starts with /admin/) uses ADMIN_SESSION_COOKIE_NAME (default 'admin_sessionid').
    Other requests use USER_SESSION_COOKIE_NAME (default is settings.SESSION_COOKIE_NAME).
    """
